...
const App = () => {
  ...
  const [selectedImageUri, setSelectedImageUri] = useState(null); // Add this state
  ...
  const pickImage = async () => {
    ...
    if (
      ...
      setSelectedImageUri(pickerResult.assets[0].uri); // Set the selected image URI
  ...
  const classifyImage = async (imageUri) => {
    ...
    try {
      let response = await fetch("http://0.0.0.0:8000/predict", {
        method: "POST",
        body: formData,
      });
  ...
  return (
    <View style={styles.container}>
      ...
      <Text style={styles.headerSubtitle}>
        A smart platform for fruit leaf disease prediction
      </Text>
      {result && (
...
const styles = StyleSheet.create({
  ...
  resultTextContainer: {
    backgroundColor: "#E0E0E0", // Different background color for resultText
    borderRadius: 10,
    padding: 10,
  },
  resultText: {
